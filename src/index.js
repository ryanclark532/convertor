...
program
  .name('convertor')
  .description(
    'A tool for converting MSSQL database data to language-specific types'
  )
  .version('1.0.0')
  .requiredOption('-c, --connection <string>', 'MSSQL connection string')
  .requiredOption('-o, --output <directory>', 'Output directory path')
  .option(
    '-q, --quicktype-options <file>',
    'JSON file containing quicktype options'
  )
  .action(async options => {
    ...
    if (quicktypeOptions) {
      try {
        ...
        console.warn(
          `Failed to load quicktype options file, using default config: ${error.message}`
        );
      }
    }
    ...
    const tableSchemas = Array.from(tableColumns, ([tableName, columns]) => {
      return JSON.stringify(
        generateJsonSchemaFromTable(tableName, columns),
        null,
        2
      );
    });
    ...
    const procSchemas = storedProcedures.map(proc => {
      ...
      return JSON.stringify(
        generateJsonSchemaFromProc(proc.name, parsed),
        null,
        2
      );
    });
